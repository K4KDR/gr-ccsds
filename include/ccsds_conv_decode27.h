...
/*! \brief Create a Convolutional decoder block and return it's shared pointer.
 *
 ...
 */
CCSDS_API ccsds_conv_decode27_sptr ccsds_make_conv_decode27 (const unsigned char gen_poly_c1, const unsigned char gen_poly_c2, ccsds_conv_encode27_punct::punct_t puncturing_type, const unsigned int block_len, std::string ASM);
 ...
 *	byte.
 */
class ccsds_conv_decode27 : public gr_sync_block {

private:
	friend CCSDS_API ccsds_conv_decode27_sptr ccsds_make_conv_decode27(const unsigned char gen_poly_c1, const unsigned char gen_poly_c2, ccsds_conv_encode27_punct::punct_t puncturing_type, const unsigned int block_len, std::string ASM);

	typedef boost::shared_ptr<float[]> softbits_sptr;

	/*! \brief Private constructor for the convolutional decoder block.
	 *
	 *  \param gen_poly_c1 Compact generator polynom for c1 stream. Default is 0x79.
	 *  \param gen_poly_c2 Compact generator polynom for c2 stream. Default is 0xDB.
	 *  \param puncturing_type Symbol for type of puncturing. Default is NONE.
	 ...
	 *  c2 the compact polynomials are 0x79 and 0xDB.
	 */
	ccsds_conv_decode27(const unsigned char gen_poly_c1, const unsigned char gen_poly_c2, ccsds_conv_encode27_punct::punct_t puncturing_type, const unsigned int block_len, std::string ASM);
	...
	static unsigned int get_rate_in(ccsds_conv_encode27_punct::punct_t puncturing_type);
	static unsigned int get_rate_out(ccsds_conv_encode27_punct::punct_t puncturing_type);

	static unsigned int get_num_inputs_req(ccsds_conv_encode27_punct::punct_t puncturing_type, const unsigned int num_out);
...
public:
	...
	~ccsds_conv_decode27(void);  // public destructor
