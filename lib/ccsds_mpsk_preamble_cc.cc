...
ccsds_make_mpsk_preamble_cc (const unsigned int num_symbols) {
    return ccsds_mpsk_preamble_cc_sptr (new ccsds_mpsk_preamble_cc (num_symbols));
}

ccsds_mpsk_preamble_cc::ccsds_mpsk_preamble_cc(const unsigned int num_symbols)
  : gr_block ("ccsds_mpsk_preamble_cc",
	gr_make_io_signature (1, 1, sizeof (gr_complex)),
	gr_make_io_signature (1, 1, sizeof (gr_complex))), d_NUM_SYMBOLS(num_symbols)
{
	d_preamble_count = 0;
