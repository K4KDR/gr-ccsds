 ...
 */
typedef boost::shared_ptr<ccsds_randomiser> ccsds_randomiser_sptr;

/*!
 *  \brief Return a shared_ptr to a new instance of ccsds_randomiser
 *
 ...
 */
CCSDS_API ccsds_randomiser_sptr ccsds_make_randomiser(const uint8_t polynomial=0x95, const uint8_t seed=0xFF);
 ...
 */
class CCSDS_API ccsds_randomiser : public gr_sync_block
{
private:
	friend CCSDS_API ccsds_randomiser_sptr ccsds_make_randomiser(const uint8_t polynomial, const uint8_t seed);
	/*  \brief Constructor of the block based pseudo (de)randomiser.
	 *
	 *  \param polynomial Polynomial that defines the network of the 8 shift
	 *	registers. The MSB encodes whether the last shift register (which is
	 *	used for the output) should be used to generate the new input while the
	 ...
	 *	all ones as defined in ECSS.
	 */
	ccsds_randomiser(const uint8_t polynomial=0x95, const uint8_t seed=0xFF);
