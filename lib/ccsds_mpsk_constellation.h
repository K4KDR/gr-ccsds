 ...
 */
inline gr_complex* ccsds_make_psk_const(unsigned int M) {
 ...
 */
inline gr_complex* ccsds_make_qpsk_const(void) {
	// allocate meory
 ...
 *	allocated with new. This array must be deallocated with delete[] after
 *	usage.
 *  \sa ccsds_make_psk_cons
 *  \sa ccsds_make_qpsk_const
 *
 *  Create a M-PSK constellation. For M=4 i.e. QPSK \ref ccsds_make_qpsk_const,
 *  otherwise \ref ccsds_make_psk_const is called
 */
inline gr_complex* ccsds_make_mpsk_const(unsigned int M) {
	//printf("\n++ Creating constelation for %u-PSK++\n",M);
	if(M == 4) {
		return ccsds_make_qpsk_const();
	} else {
		return ccsds_make_psk_const(M);
	}
}
 ...
 */
inline unsigned char* ccsds_make_psk_map(unsigned int M) {
 ...
 *  According to ECSS-E-ST-50-05C Rev.2 from 4th October 2011, Figure 6-3
 */
inline unsigned char* ccsds_make_qpsk_map(void) {
	// allocate meory
 ...
 *  \ref ccsds_make_qpsk_map, otherwise \ref ccsds_make_psk_map is called
 */
inline unsigned char* ccsds_make_mpsk_map(unsigned int M) {
	if(M == 4) {
		return ccsds_make_qpsk_map();
	} else {
		return ccsds_make_psk_map(M);
	}
}
 ...
 *  output.
 */
inline unsigned char* ccsds_make_psk_rev_map(unsigned int M) {
	// allocate memory for the constellation mapping
	unsigned char *map = new unsigned char[M];
 ...
 *  According to ECSS-E-ST-50-05C Rev.2 from 4th October 2011, Figure 6-3
 */
inline unsigned char* ccsds_make_qpsk_rev_map(void) {
	// allocate meory
	unsigned char *map = new unsigned char[4];
 ...
 *  \ref ccsds_make_qpsk_rev_map, otherwise \ref ccsds_make_psk_rev_map is called
 */
inline unsigned char* ccsds_make_mpsk_rev_map(unsigned int M) {
	if(M == 4) {
		return ccsds_make_qpsk_rev_map();
	} else {
		return ccsds_make_psk_rev_map(M);
	}
}
